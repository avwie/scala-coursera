...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(
    ...
    for {
      (b2, m) <- b.legalNeighbors.to(LazyList)
    } yield (b2, m :: history)
  ...
  def newNeighborsOnly(
      ...
      explored: Set[Block]
  ): LazyList[(Block, List[Move])] = neighbors.filter {
  ...
  def from(
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val explored2 = explored ++ neighbors.map(n => n._1)
      initial.head #:: from(initial.tail ++ neighbors, explored2)
    }
  }
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = {
    val start: LazyList[(Block, List[Move])] =
      (startBlock, List.empty) #:: LazyList.empty
